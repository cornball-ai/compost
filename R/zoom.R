...
#'
#' @export
zoom <- function(input, output, stt, amount = 0.05, levels = 3,
                 transition = 0.3, seed = NULL, width = NULL, height = NULL,
    ...
    args <- c(if (overwrite) "-y", "-i", input, "-vf", filter, "-c:v",
              "libx264", "-preset", "fast", "-c:a", "copy", output)
...
.build_zoom_expr <- function(frame_starts, zoom_values, transition_frames) {
    ...
    seg_expr <- function(i) {
        ...
        sprintf("%.4f+%.4f*min((on-%d)/%d,1)", z_prev, z - z_prev,
                frame_starts[i], transition_frames)
    }
